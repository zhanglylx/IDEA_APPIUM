    ...
    public void buttonMouseListener(JButton f) {
        f.addActionListener(e -> {
            ...
            if(f.getText().startsWith("save")){
                Utlis.WindosUtils.copyFile(this,FlowConfig.fileSit + getTitle() + File.separator +
                        f.getText().substring( f.getText().indexOf("save")+4, f.getText().length()),f);
            }else{
                f.setEnabled(false);
                Utlis.WindosUtils.openFile(FlowConfig.fileSit + getTitle() + File.separator + f.getText());
